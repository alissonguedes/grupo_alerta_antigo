    ...
    @foreach ($paginate as $row)
        ...
        <tr class="{{ $row->status === '0' ? 'blocked' : null }}" id="{{ $row->id }}" data-disabled="false">
            ...
            <td width="1%" data-disabled="true">
                <label>
                    <input type="checkbox" name="id[]" value="{{ $row->id }}" data-status="{{ $row->status }}">
                    <span></span>
                </label>
            </td>
            <td>{{ $row->label }}</td>
            ...
            <td class="center-align" data-disabled="true">
                <button data-href="{{ route('admin.menus.edit', $row->id) }}"
                    class="btn btn-flat btn-edit btn-floating float-none">
                </button>
            </td>
        </tr>
        ...
        <ul>
            <li>
                <button class="btn btn-flat btn-floating waves-effect"
                    ...
                    data-tooltip="Anterior" {{ $paginate->onFirstPage() ? 'disabled' : null }}>
